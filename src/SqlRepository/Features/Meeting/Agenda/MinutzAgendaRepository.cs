...
namespace SqlRepository.Features.Meeting.Agenda
{
    public class MinutzAgendaRepository : IMinutzAgendaRepository
    {
                        ...
                        : new MessageBase
                          {
                              Code = 404,
                              Condition = false,
                              Message = "Could not update agenda isComplete."
                          };
                }
            }
            catch (Exception e)
                        ...
                        : new MessageBase
                          {
                              Code = 404,
                              Condition = false,
                              Message = "Could not update agenda duration."
                          };
                }
            }
            catch (Exception e)
        ...
        public MessageBase UpdateTitle(Guid agendaId, string title, string schema, string connectionString)
        {
            ...
            try
            {
                using (IDbConnection dbConnection = new SqlConnection(connectionString))
                {
                    dbConnection.Open();
                    var sql =
                        $"UPDATE [{schema}].[MeetingAgenda] SET [AgendaHeading] = '{title}' WHERE Id = '{agendaId}'";
                    var data = dbConnection.Execute(sql);
                    return data == 1
                        ? new MessageBase {Code = 200, Condition = true, Message = "Success"}
        ...
        public MessageBase UpdateAssignedAttendee(Guid agendaId, string attendeeEmail, string schema, string connectionString)
        {
            ...
            try
            {
                using (IDbConnection dbConnection = new SqlConnection(connectionString))
                {
                    dbConnection.Open();
                    var sql =
                        $"UPDATE [{schema}].[MeetingAgenda] SET [MeetingAttendeeId] = '{attendeeEmail}' WHERE Id = '{agendaId}'";
                    var data = dbConnection.Execute(sql);
                    return data == 1
                        ? new MessageBase {Code = 200, Condition = true, Message = "Success"}
                        : new MessageBase
                          {
                              Code = 404,
                              Condition = false,
                              Message = "Could not update agenda assigned attendee."
                          };
                }
            }
            catch (Exception e)
        ...
        public AgendaMessage QuickCreate(string meetingId, string agendaTitle, int order, string schema, string connectionString)
        {
            ...
            try
            {
                using (IDbConnection dbConnection = new SqlConnection(connectionString))
                {
                    ...
                    dbConnection.Open();
                    var insertSql =
                        $@"INSERT INTO [{schema}].[MeetingAgenda]([Id],[ReferanceId],[AgendaHeading],[Order]) 
                            ...
                            return new AgendaMessage
                                   {
                                       Code = 404,
                                       Condition = false,
                                       Message = "Could not find quick create agenda item."
                                   };
                        return new AgendaMessage
                               {
                                   Code = 200,
                    ...
                    return new AgendaMessage
                           {
                               Code = 404,
                               Condition = false,
                               Message = "Could not quick create agenda."
                           };
                }
            }
            catch (Exception e)
        ...
        public AgendaMessage GetMeetingAgendaCollection(Guid meetingId, string schema, string connectionString)
        {
